...
// 🌐 中文注释(汉化仓库添加):本 crate 实现 .env 环境文件的发现、
// 读取与解析,是 `--env-file` 功能的核心库。
//
// 结构一览:
// - `candidate_paths` / `find_path_and_content`:
//     从 cwd 沿祖先目录向上查找 env 文件并读出内容;
// - `parse_env_content_hook`(移植自 Node.js 的 node_dotenv.cc):
//     逐行解析 `KEY=VALUE`,支持 `#` 注释、单/双/反引号引值、
//     多行值与转义;通过回调把键值对交给调用方;
// - `parse_env_content_with_substitution_hook`:
//     在上述基础上支持 `$VAR` / `${VAR}` 变量替换(值可引用此前定义的
//     变量或真实环境变量),并缓存替换结果供后续引用;
// - `from_content_sanitized_iter_with_substitution`:
//     包装为迭代器,并把包含 `\0` 的键值对报告为 `ParseError`。
use std::borrow::Cow;
...
/// 返回给定 env 文件描述符的候选路径迭代器。
///
/// 中文注释(汉化仓库添加):描述符为绝对路径时只产出该路径;
/// 否则从 `cwd` 开始沿祖先目录逐级向上,把描述符拼到每一级目录上。
...
/// 发现 env 文件的路径与内容,供后续解析。
///
/// 中文注释(汉化仓库添加):沿祖先目录逐级尝试读取 env 文件;
/// 找到即返回(路径已归一化),全部不存在则返回 `Ok(None)`,
/// 读取失败(非 NotFound)会带上出错路径报错。
...
// 中文注释(汉化仓库添加):把解析结果包装为迭代器;
// 键或值中含 `\0` 视为非法,报告为 ParseError。
...
// 中文注释(汉化仓库添加):基础解析入口,不做变量替换;
// 每解析出一对 (key, value) 就回调一次 cb。
pub fn parse_env_content_hook(content: &str, cb: &mut dyn FnMut(&str, &str)) {
  parse_env_content_hook_impl(content, None, cb);
}

// 中文注释(汉化仓库添加):带 `$VAR`/`${VAR}` 变量替换的解析入口;
// SubstitutionMap 会缓存已产出的键值,供后续值中的引用使用,
// 未定义的变量回退到真实环境变量(sys)。
